...
- **점수/최고기록/다이아몬드**: 점수는 0에서 시작해 매 1초마다 `floor(현재 속도 km/h)` 누적(1점 단위). **최고 기록(BEST)** 은 `localStorage` 에 저장되어 재시작 후에도 유지되고, 현재 점수가 넘으면 함께 올라간다. 다이아몬드(◆)는 5개로 시작, **충돌마다 1개 소멸**(빈 ◇로 표시), 다 사라지면 **GAME OVER** + Restart 버튼(처음부터 다시).
- **일시정지**: cockpit 뷰 중앙의 ⏸ 버튼으로 게임을 멈추고(⏸↔▶) 재개(오디오 포함).
