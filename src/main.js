...
const SCENE_EXPOSURE = 0.8;       // 기본 노출(전체 화면을 더 밝게)
...
function makeCloudTexture() {
  ...
  for (let i = 0; i < puffCount; i++) {
    ...
    const r = N * (0.07 + Math.random() * 0.13);           // 큰 덩어리(경계 안쪽에 머물게)
    const a = 0.06 + Math.random() * 0.11;                 // 더 낮춘 알파 → 천천히 누적(밝기 단계 형성)
    // 순백(255) 대신 옅은 회백색을 가산. 겹치는 정도(밀도)에 따라 밝기가 단계적으로 쌓이고,
    // 한 번에 흰색으로 포화되지 않아 누적된 볼륨마다 밝기가 다양하게 드러난다.
    const grad = g.createRadialGradient(x, y, 0, x, y, r);
    grad.addColorStop(0, `rgba(214,219,226,${a})`);
    grad.addColorStop(1, 'rgba(214,219,226,0)');
    g.fillStyle = grad;
    g.beginPath();
...
const skyTint = new THREE.Mesh(
  ...
  new THREE.MeshBasicMaterial({
    color: 0x2a66cc, side: THREE.BackSide, transparent: true, opacity: 0.8, // 짙은 청색 틴트(조금 밝게)
...
const COLLISION_DIST_FRAC = 0.42; // 충돌(스핀/복귀) 중 추격 거리 축소 비율 — 차량에 근접 관찰
const COLLISION_CAM_RATE = 3.0;   // 추격 거리 전환 속도(부드럽게)
...
function checkCollisions() {
  ...
  for (const t of traffic) {
    ...
    startSpin(t, -heroCCW * SPIN_OMEGA0, t.heading, drive.speed * t.speedRatio * t.ramp);
    playCrashSound(); // 충돌음
...
window.addEventListener('keydown', (e) => {
  ...
  // ESC: 게임 중단(일시정지)으로 진입 / 다시 누르면 재개. 진행 중일 때만, 카운트다운·정지(F) 제외.
...
function animate() {
  ...
  if (drive.active && !game.paused && game.countdown <= 0) {
    ...
    if (drive.state === 'drive') {
      ...
      orientToTerrain(car, car.position.x, car.position.z, drive.heading); // 노면 기울기로 피치·롤
    ...
    // 충돌(스핀/복귀) 중에는 거리를 좁혀 차량에 근접, 회복 과정을 가까이서 관찰한다.
    const distTarget = drive.state !== 'drive'
      ? camFollow.chaseDist * COLLISION_DIST_FRAC
      : camFollow.chaseDist;
  ...
  if (cockpitReady) {
    ...
    _tFwd.set(Math.cos(drive.heading), 0, -Math.sin(drive.heading));
