...
function buildTrack(radius, roadHalfWidth) {
  ...
  // 도로 중앙 차선(흰색 점선) — 가운데 한 줄. 곡선을 LANE_CELLS 칸으로 나눠 각 칸의
  // 앞부분(LANE_DASH_FRAC)만 칠해 대시-빈칸이 반복되는 점선을 만든다.
  const LANE_CELLS = 120;                       // 점선 칸 수(많을수록 짧고 촘촘한 대시)
  const LANE_DASH_FRAC = 0.45;                  // 한 칸에서 대시가 차지하는 비율(나머지는 빈칸)
  const laneHalfW = roadHalfWidth * 0.04;       // 차선 폭(중앙선 기준 좌우 ±)
  const lPos = [], lIdx = [];
  ...
  for (let i = 0; i < LANE_CELLS; i++) {
    ...
    for (const [u, off] of [[u0, -laneHalfW], [u0, laneHalfW], [u1, -laneHalfW], [u1, laneHalfW]]) {
      curve.getPointAt(u, lp);
      curve.getTangentAt(u, ltan);
      llat.crossVectors(up, ltan).normalize();
      lPos.push(lp.x + llat.x * off, 0.04, lp.z + llat.z * off); // 도로(0.02)·가장자리(0.03) 살짝 위
    }
...
function updatePeople(dt) {
  ...
  for (const p of people) {
    ...
    p.rig.position.set(_pos.x + _lat.x * p.lat, 0, _pos.z + _lat.z * p.lat);
