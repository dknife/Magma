...
const SCENE_EXPOSURE = 0.62;      // 기본 노출
...
function resumeAudio() {
  ...
  if (!ctx) return;
  if (ctx.state === 'suspended' && !game.paused && !game.over) ctx.resume(); // 정지/게임오버 중엔 깨우지 않음
  initEngine();
}
...
// 충돌/스핀/복귀 튜닝
const SPIN_OMEGA0 = 6.5;        // 충돌 직후 회전 각속도(rad/s) — 조금 더 강하게
const SPIN_OMEGA_DECAY = 3.0;   // 회전 감쇠율(1/s) — 낮춰서 회전이 조금 더 오래
const SPIN_OMEGA_STOP = 0.3;    // 이 각속도 미만이면 스핀 종료로 보고 복귀
const SPIN_SPEED_DECAY = 18.0;  // 진행 속도 감쇠율(1/s) — 낮춰서 감속을 조금 더 지연
...
// 점수 / 게임오버
// ---------------------------------------------------------------------------
const game = { score: 0, sec: 0, diamonds: MAX_DIAMONDS, best: 0, over: false, paused: false };
...
function togglePause() {
  ...
  const ctx = getAudioCtx();
  if (ctx) { if (game.paused) ctx.suspend(); else ctx.resume(); }
}
function gameOver() {
  game.over = true;
...
function animate() {
  ...
  if (drive.active && !game.paused) {
    ...
    if (speedValEl && needleEl && drive.maxSpeed > 0) {
      const groundSpeed = drive.state === 'drive' ? drive.speed : drive.slideSpeed;
    ...
    if (game.sec >= 1) {
      game.sec -= 1;
      const gs = drive.state === 'drive' ? drive.speed : drive.slideSpeed;
  ...
  // 단일 패스로 씬(레이어 0) + 전시 모델(레이어 2)을 같이 그린다(모델은 기울여 회전).
  const showcasing = (game.paused || game.over) && showcaseGroup;
  if (showcasing) {
    ...
    renderer.toneMappingExposure = SCENE_EXPOSURE;       // 복구(미니맵 패스용)
  } else {
    renderer.render(scene, camera);
  }
