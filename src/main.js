...
let boards = (() => {
  try { const b = JSON.parse(localStorage.getItem(BOARDS_KEY)); if (b && Array.isArray(b.score) && Array.isArray(b.lap)) { if (!Array.isArray(b.finish)) b.finish = []; return b; } } catch {}
  return { score: [], lap: [], finish: [] };
})();
...
// 경기 결과 등록: 스코어(내림차순)·베스트랩(오름차순)·완주시간(오름차순) 세 보드에 Top10 이내면 기록.
function submitResult(name, score, lap, time) {
  ...
  const fRank = time > 0 ? boards.finish.filter((e) => e.time > 0 && e.time < time).length + 1 : Infinity;
  let sIn = false, lIn = false, fIn = false;
  ...
  if (time > 0 && fRank <= 10) { boards.finish.push({ ...entry }); boards.finish.sort((a, b) => a.time - b.time); boards.finish = boards.finish.slice(0, 10); fIn = true; }
  if (sIn || lIn || fIn) saveBoards();
  return { sRank, lRank, fRank, sIn, lIn, fIn };
}
...
onTap(introScreenEl, (e) => {
  ...
  // 순위표는 차량 선택이 아니라 이후 Hall of Fame 장면에서 보여 준다(여기선 표시하지 않음).
});
...
const godashBtn = document.getElementById('godash-btn');
const backSelectBtn = document.getElementById('back-select-btn');
// START: 이름 확정 후 Hall of Fame(스코어보드) 장면으로 전환(아직 레이싱 시작 아님)
...
onTap(godashBtn, startGame);     // Go Dash! → 레이싱 시작
onTap(backSelectBtn, backToSelect);
// 모바일 순위표 좌우 토글(베스트랩 ↔ 스코어 ↔ 완주시간)
const lbPrevBtn = document.getElementById('lb-prev');
const lbNextBtn = document.getElementById('lb-next');
onTap(lbNextBtn, () => { lbViewIdx = (lbViewIdx + 1) % LB_VIEWS.length; applyMobileLbView(); });
onTap(lbPrevBtn, () => { lbViewIdx = (lbViewIdx + LB_VIEWS.length - 1) % LB_VIEWS.length; applyMobileLbView(); });
...
if (minimapEl) {
  ...
  // 모바일: Score/Best/다이아몬드는 작은 창과 에너지 게이지 사이 공간에 가운데 정렬(CSS) — DOM 이동 없음
...
const finFRankEl = document.getElementById('fin-frank');
...
const lbFinishBody = document.getElementById('lb-finish-body');
const lbMobileTitle = document.getElementById('lb-mobile-title');
const LB_VIEWS = ['lap', 'score', 'finish'];      // 모바일 토글 순서(기본 베스트랩)
const LB_TITLES = { lap: 'BEST LAP TOP 5', score: 'SCORE TOP 5', finish: 'FINISH TIME TOP 5' };
let lbViewIdx = 0;
...
function boardRows(list, kind, max) {
  ...
  for (let i = 0; i < max; i++) {
    ...
    const val = !e ? '—' : kind === 'score' ? e.score : kind === 'finish' ? fmtTime(e.time) : fmtLap(e.lap);
...
function applyMobileLbView() {
  ...
  const v = LB_VIEWS[lbViewIdx];
  leaderboardEl.classList.toggle('show-score', v === 'score');
  leaderboardEl.classList.toggle('show-lap', v === 'lap');
  leaderboardEl.classList.toggle('show-finish', v === 'finish');
  if (lbMobileTitle) lbMobileTitle.textContent = LB_TITLES[v];
}
function updateLeaderboards() {
  ...
  if (lbFinishBody) lbFinishBody.innerHTML = boardRows(boards.finish, 'finish', max);
...
function finishStop() {
  ...
  if (finFRankEl) finFRankEl.textContent = res.fRank === Infinity ? '—' : `#${res.fRank}`;
  if (finPointsEl) {
    const eligible = res.sIn || res.lIn || res.fIn;
